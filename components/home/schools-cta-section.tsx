...
export function SchoolsCtaSection() {
  ...
  return (
    <section style={{ padding: "40px 40px 60px" }} className="flex justify-center bg-[#F7F9FC]">
      <div style={{ padding: "60px 40px" }} className="relative w-full overflow-hidden rounded-3xl">
        ...
        <div className="pointer-events-none absolute inset-0">
          {[
            ...
            <div key={i} className="absolute flex items-center justify-center text-white/40" style={{ left: icon.left, top: icon.top, width: icon.size, height: icon.size, transform: `rotate(${icon.rotate}deg)` }}>
        ...
        <div className="relative flex flex-col items-center text-center">
          <h2 style={{ marginBottom: 28 }} className="text-3xl leading-tight text-white lg:text-4xl">
            {isHe ? "בתי ספר, גייסו את המורים" : "Schools, recruit the teachers"}
            <br />
            {isHe ? "!שאתם מחפשים היום" : "you are looking for today!"}
          </h2>
          <button style={{ padding: "14px 40px" }} className="cursor-pointer rounded-xl border-none bg-white text-sm text-primary transition-colors hover:bg-white/90">
            {isHe ? "הטבה מיוחדת לבתי ספר" : "Special benefit for schools"}
          </button>
        </div>
      </div>
    </section>
