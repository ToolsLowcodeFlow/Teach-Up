...
function JobDetailPage() {
  ...
  const rejectedSteps = [
    { label: jd.newCandidacy, status: "done" },
    { label: jd.invitedFirstInterview, status: "done" },
    { label: jd.calledSecondInterview, status: "done" },
    { label: jd.rejected, status: "rejected" },
  ];
  ...
  return (
    ...
    >
      <div className="fixed inset-0" style={{ background: "rgba(0,0,0,0.4)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)" }} onClick={() => router.push(applied ? "/jobs?tab=my" : "/jobs")} />
      ...
      >
        ...
        <div className="flex flex-1 flex-col">
          ...
          <div className="flex flex-col" style={{ marginBottom: 10 }}>
            <div className="flex items-center justify-between">
              ...
              <div className="flex cursor-pointer items-center gap-2">
                <span className="text-sm" style={{ color: "#FF6B6B" }}>{jd.removeFromFavorites}</span>
          ...
          <div className="flex items-center gap-4 text-xs" style={{ marginBottom: 20 }}>
            ...
            <span className="text-muted-foreground">{jd.jobNumber} · 84844985</span>
          </div>
          ...
          <div className="rounded-[10px] border border-border-light" style={{ padding: "14px 18px", marginBottom: 20, background: "linear-gradient(180deg, #F7FAFF 0%, #EEF4FD 100%)" }}>
            <div className="grid grid-cols-5 gap-x-4 gap-y-4 text-center">
              {[
                [jd.hoursOfOperation, "Lunch / Dinner"],
                [jd.role, "Lorem Ipsum"],
                [jd.transactionType, "Lorem Ipsum"],
                [jd.educationStage, "Lorem Ipsum"],
                [jd.startDate, "08/10/2026"],
                [jd.training, "Morning training"],
                [jd.yearsOfExperience, "09"],
                [jd.scopeOfWork, "Full-time"],
                [jd.area, "Tel Aviv, Israel"],
                [jd.salaryRange + " ₪", "10,000 - 20,000"],
              ].map(([label, value]) => (
          ...
          <div className="flex flex-col" style={{ gap: 8, marginBottom: 20 }}>
            <h3 className="text-base text-foreground">{jd.companyDescription}</h3>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-foreground">{jd.companyName}: Lor Ipsum</span>
            </div>
          </div>

          <div className="flex flex-col" style={{ gap: 8 }}>
            <h3 className="text-base text-foreground">{jd.attachments}</h3>
            {[1, 2, 3].map((i) => (
        ...
        <div className="flex shrink-0 flex-col border-l border-border-light" style={{ width: 280, padding: "20px" }}>
          ...
          <div className="flex flex-col rounded-2xl bg-white" style={{ padding: "22px 16px", gap: 16, boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }}>
            {applied ? (
              ...
              <>
                <h3 className="text-center text-[18px] text-foreground" style={{ lineHeight: 1.2 }}>{jd.interestedInJob}</h3>
                <p className="text-center text-xs leading-[1.4] text-muted-foreground">
                  {jd.candidacyStatusDesc}
                </p>
                <button
